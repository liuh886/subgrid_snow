            ...
            for beam_ind, beam in enumerate(beams):
                ...
                for group in dataset_dict.keys():
                    for dataset in dataset_dict[group]:
                        ...
                        # since a dataset may not exist in a file, we're going to try to read it, and if it doesn't work, we'll move on to the next:
                        try:
                            if dataset in ['h_te_best_fit_20m','latitude_20m','longitude_20m']:
                                _temp = np.array(h5f[DS])
                                _temp[_temp == h5f[DS].attrs['_FillValue']] = np.NaN
                                while _temp.shape[1] !=5:
                                    _temp = np.c_[_temp,np.zeros_like(_temp[:,1])*np.nan]
                                try:
                                    temp[dataset+'_0'] = _temp[:,0].T
                                    temp[dataset+'_1'] = _temp[:,1].T
                                    temp[dataset+'_2'] = _temp[:,2].T
                                    temp[dataset+'_3'] = _temp[:,3].T
                                    temp[dataset+'_4'] = _temp[:,4].T
                                except IndexError:
                                    pass
                            elif dataset == 'subset_te_flag':
                                _temp = np.array(h5f[DS], dtype=np.float32)
                                _temp[_temp == h5f[DS].attrs['_FillValue']] = np.NaN
                            ...
                            if '_FillValue' in h5f[DS].attrs:
                                fill_value=h5f[DS].attrs['_FillValue']
                                if dataset not in ['h_te_best_fit_20m','subset_te_flag']:
                                    bad = temp[dataset]==fill_value
                                    if True in bad:
                                        temp[dataset]=np.float64(temp[dataset])
                                        temp[dataset][bad]=np.NaN
...
def ATL08_2_gdf(ATL06_fn,dataset_dict):
    ...
    for track in data_dict:
        #1 track
        #convert to datafrmae. ZHIHAO: this is a problem
